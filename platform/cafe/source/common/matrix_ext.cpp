...
void Matrix4<Console::CAFE>::Multiply(const Matrix4& a, const Matrix4& b, float elements[16])
{
    elements[0] = (a.elements[0] * b.elements[0]) + (a.elements[4] * b.elements[1]) +
                  (a.elements[8] * b.elements[2]) + (a.elements[12] * b.elements[3]);
    elements[4] = (a.elements[0] * b.elements[4]) + (a.elements[4] * b.elements[5]) +
                  (a.elements[8] * b.elements[6]) + (a.elements[12] * b.elements[7]);
    elements[8] = (a.elements[0] * b.elements[8]) + (a.elements[4] * b.elements[9]) +
                  (a.elements[8] * b.elements[10]) + (a.elements[12] * b.elements[11]);
    elements[12] = (a.elements[0] * b.elements[12]) + (a.elements[4] * b.elements[13]) +
                   (a.elements[8] * b.elements[14]) + (a.elements[12] * b.elements[15]);

    elements[1] = (a.elements[1] * b.elements[0]) + (a.elements[5] * b.elements[1]) +
                  (a.elements[9] * b.elements[2]) + (a.elements[13] * b.elements[3]);
    elements[5] = (a.elements[1] * b.elements[4]) + (a.elements[5] * b.elements[5]) +
                  (a.elements[9] * b.elements[6]) + (a.elements[13] * b.elements[7]);
    elements[9] = (a.elements[1] * b.elements[8]) + (a.elements[5] * b.elements[9]) +
                  (a.elements[9] * b.elements[10]) + (a.elements[13] * b.elements[11]);
    elements[13] = (a.elements[1] * b.elements[12]) + (a.elements[5] * b.elements[13]) +
                   (a.elements[9] * b.elements[14]) + (a.elements[13] * b.elements[15]);

    elements[2] = (a.elements[2] * b.elements[0]) + (a.elements[6] * b.elements[1]) +
                  (a.elements[10] * b.elements[2]) + (a.elements[14] * b.elements[3]);
    elements[6] = (a.elements[2] * b.elements[4]) + (a.elements[6] * b.elements[5]) +
                  (a.elements[10] * b.elements[6]) + (a.elements[14] * b.elements[7]);
    elements[10] = (a.elements[2] * b.elements[8]) + (a.elements[6] * b.elements[9]) +
                   (a.elements[10] * b.elements[10]) + (a.elements[14] * b.elements[11]);
    elements[14] = (a.elements[2] * b.elements[12]) + (a.elements[6] * b.elements[13]) +
                   (a.elements[10] * b.elements[14]) + (a.elements[14] * b.elements[15]);

    elements[3] = (a.elements[3] * b.elements[0]) + (a.elements[7] * b.elements[1]) +
                  (a.elements[11] * b.elements[2]) + (a.elements[15] * b.elements[3]);
    elements[7] = (a.elements[3] * b.elements[4]) + (a.elements[7] * b.elements[5]) +
                  (a.elements[11] * b.elements[6]) + (a.elements[15] * b.elements[7]);
    elements[11] = (a.elements[3] * b.elements[8]) + (a.elements[7] * b.elements[9]) +
                   (a.elements[11] * b.elements[10]) + (a.elements[15] * b.elements[11]);
    elements[15] = (a.elements[3] * b.elements[12]) + (a.elements[7] * b.elements[13]) +
                   (a.elements[11] * b.elements[14]) + (a.elements[15] * b.elements[15]);
}

void Matrix4<Console::CAFE>::Multiply(const Matrix4& a, const Matrix4& b, Matrix4& t)
{
    Matrix4<Console::CAFE>::Multiply(a, b, t.elements);
}

Matrix4<Console::CAFE>::Matrix4()
{
    this->SetIdentity();
}

Matrix4<Console::CAFE>::Matrix4(const float elements[16])
{
    memcpy(this->elements, elements, sizeof(float) * 16);
}

Matrix4<Console::CAFE>::Matrix4(const Matrix4& a, const Matrix4& b)
{
    Matrix4<Console::CAFE>::Multiply(a, b, this->elements);
}

Matrix4<Console::CAFE>::Matrix4(float t00, float t10, float t01, float t11, float x, float y)
{
    this->SetRawTransformation(t00, t10, t01, t11, x, y);
}

Matrix4<Console::CAFE>::Matrix4(float x, float y, float angle, float sx, float sy, float ox,
                                float oy, float kx, float ky)
{
    this->SetTransformation(x, y, angle, sx, sy, ox, oy, kx, ky);
}

bool Matrix4<Console::CAFE>::IsAffine2DTransform() const
{
...
Matrix4<Console::CAFE> Matrix4<Console::CAFE>::operator*(const Matrix4& m) const
{
    return Matrix4(*this, m);
}

void Matrix4<Console::CAFE>::operator*=(const Matrix4& m)
{
    float elements[16];

    Matrix4<Console::CAFE>::Multiply(*this, m, elements);
    memcpy(this->elements, elements, sizeof(float) * 16);
}

void Matrix4<Console::CAFE>::SetIdentity()
{
    memset(this->elements, 0, sizeof(float) * 16);
    this->elements[15] = this->elements[10] = this->elements[5] = this->elements[0] = 1.0f;
}

void Matrix4<Console::CAFE>::SetTranslation(float x, float y)
{
    this->SetIdentity();
...
void Matrix4<Console::CAFE>::Translate(float x, float y)
{
    Matrix4 t;
...
void Matrix4<Console::CAFE>::SetRotation(float rad)
{
    this->SetIdentity();
...
void Matrix4<Console::CAFE>::Rotate(float rad)
{
    Matrix4 t;
    t.SetRotation(rad);
    this->operator*=(t);
...
void Matrix4<Console::CAFE>::SetScale(float sx, float sy)
{
    this->SetIdentity();
...
void Matrix4<Console::CAFE>::Scale(float sx, float sy)
{
    Matrix4 t;
    t.SetScale(sx, sy);
    this->operator*=(t);
...
void Matrix4<Console::CAFE>::SetShear(float kx, float ky)
{
    this->SetIdentity();
...
void Matrix4<Console::CAFE>::Shear(float kx, float ky)
{
    Matrix4 t;
    t.SetShear(kx, ky);
    this->operator*=(t);
...
void Matrix4<Console::CAFE>::GetApproximateScale(float& sx, float& sy) const
{
...
void Matrix4<Console::CAFE>::SetRawTransformation(float t00, float t10, float t01, float t11,
                                                  float x, float y)
{
...
void Matrix4<Console::CAFE>::SetTransformation(float x, float y, float angle, float sx, float sy,
...
Matrix4<Console::CAFE> Matrix4<Console::CAFE>::Inverse() const
{
    Matrix4<Console::CAFE> inv;
...
Matrix4<Console::CAFE> Matrix4<Console::CAFE>::Ortho(float left, float right, float bottom,
