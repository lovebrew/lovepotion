...
namespace love
{
    ...
    /* use with Vector2 */
    template<typename Vdst, typename Vsrc>
    void Matrix4<Console::CAFE>::TransformXY(Vdst* dst, const Vsrc* src, int size) const
    {
    ...
    /* use with Vertex */
    template<typename Vdst, typename Vsrc>
    void Matrix4<Console::CAFE>::TransformXYVert(Vdst* dst, const Vsrc* src, int size) const
    {
        for (int i = 0; i < size; i++)
        {
            // Store in temp variables in case src = dst
