...
#include "driver/display/deko3d/CIntrusiveList.h"
...
class CMemPool
{
    ...
    struct Slice
    {
        ...
        constexpr bool canCoalesce(Slice const& rhs) const
        {
            return m_pool == rhs.m_pool && m_block == rhs.m_block && m_end == rhs.m_start;
        }

        constexpr bool operator<(Slice const& rhs) const
        {
  ...
  public:
    ...
    CMemPool(dk::Device dev,
             uint32_t flags     = DkMemBlockFlags_CpuUncached | DkMemBlockFlags_GpuCached,
