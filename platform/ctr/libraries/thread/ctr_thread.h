...
namespace ctr
{
    ...
    class thread
    {
      ...
      private:
        ...
        static void trampoline(void* v_arg)
        {
            ...
            std::apply(thread_arg->first,
                       std::forward<typename P_t::second_type>(thread_arg->second));
        }
      ...
      public:
        ...
        {
            static_assert(std::is_invocable_v<F, std::unwrap_reference_t<Args>...>,
                          "Can not call this function with these argument.");
            using P_t = std::pair<F&&, std::tuple<std::unwrap_reference_t<Args>...>>;
            auto arg  = std::make_unique<P_t>(std::forward<F>(func),
                                             std::forward_as_tuple(std::forward<Args>(args)...));
            m_th = threadCreate(&thread::trampoline<P_t>, arg.get(), info.stack_size, info.prio,
                                info.core_id, false);
            if (m_th)
            {
        ...
        template<class F, typename... Args,
                 typename = std::enable_if_t<!std::is_same_v<meta, std::remove_cvref_t<F>>>>
        ...
        thread(thread&& other) : m_th(std::exchange(other.m_th, nullptr))
        {}
        ...
        thread& operator=(thread&& other)
        {
            ...
            {
                if (joinable())
                    throw std::runtime_error("Moving to a thread already running.");
    ...
    class condition_variable
    {
      ...
      public:
        ...
        {
            const int r = CondVar_WaitTimeout(
                &m_cv, lock.mutex()->native_handle(),
                std::chrono::duration_cast<std::chrono::nanoseconds>(rel_time).count());
            return r == 0 ? cv_status::no_timeout : cv_status::timeout;
        }

        template<class Rep, class Period, class Predicate>
        bool wait_for(std::unique_lock<mutex>& lock,
                      const std::chrono::duration<Rep, Period>& rel_time, Predicate stop_waiting)
        {
