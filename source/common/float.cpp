...
namespace love
{
    ...
    static inline float asfloat32(uint32_t u)
    {
        union
        {
            float f;
    ...
    {
        uint32_t u = asuint32(f);
        return basetable[(u >> 23) & 0x1FF] + ((u & 0x007FFFFF) >> shifttable[(u >> 23) & 0x1FF]);
    }

    // Adapted from
    // https://stackoverflow.com/questions/41532085/how-to-pack-unpack-11-and-10-bit-floats-in-javascript-for-webgl2

    float float11to32(float11_t f)
    {
        uint32_t exponent = f >> 6;
        uint32_t mantissa = f & 0x3F;
    ...
    float11_t float32to11(float f)
    {
        ...
        int32_t exponent  = (int32_t)((u >> 23) & 0xFF) - 127;
        ...
        return ((uint16_t)exponent << EXPONENT_SHIFT) | (mantissa >> MANTISSA_SHIFT);
    }

    float float10to32(float10_t f)
    {
        uint16_t exponent = f >> 5;
        uint16_t mantissa = f & 0x1F;
