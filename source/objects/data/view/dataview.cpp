...
DataView::DataView(Data * data, size_t offset, size_t size) : data(data),
                                                              offset(offset),
                                                              size(size)
{
    if (offset >= data->GetSize() || size > data->GetSize() || offset > data->GetSize() - size)
        throw love::Exception("Offset and size of DataView must fit within the original Data's size!");

    if (size == 0)
        throw love::Exception("DataView size must be grater than zero!");
}

DataView::DataView(const DataView & other) : data(other.data),
                                             offset(other.offset),
                                             size(other.size)
{}
