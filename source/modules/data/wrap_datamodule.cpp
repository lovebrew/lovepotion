...
int Wrap_DataModule::NewByteData(lua_State * L)
{
    ...
    if (Luax::IsType(L, 1, Data::type))
    {
        ...
        Luax::CatchException(L, [&]() {
            byteData = instance()->NewByteData(bytes, (size_t)size);
        });
    }
    else if (lua_type(L, 1) == LUA_TSTRING)
    {
        ...
        Luax::CatchException(L, [&]() {
            byteData = instance()->NewByteData(data, size);
        });
    }
    else
    {
        ...
        Luax::CatchException(L, [&]() {
            byteData = instance()->NewByteData((size_t)size);
        });
    }
...
int Compress(lua_State * L)
{
    ...
    if (!Compressor::GetConstant(formatStr, format))
        return Luax::EnumError(L, "compressed data format", Compressor::GetConstants(format), formatStr);
    ...
    Luax::CatchException(L, [&]() {
        compressedData = data::_Compress(format, rawBytes, rawSize, level);
    });
...
int Decompress(lua_State * L)
{
    ...
    if (Luax::IsType(L, 2, CompressedData::type))
    {
        ...
        Luax::CatchException(L, [&]() {
            rawBytes = data::_Decompress(data, rawSize);
        });
    }
    else
    {
    ...
    if (type == data::CONTAINER_DATA)
    {
        ...
        Luax::CatchException(L, [&]() {
            data = instance()->NewByteData(rawBytes, rawSize, true);
        });
...
int Wrap_DataModule::Decode(lua_State * L)
{
    ...
    Luax::CatchException(L, [&]() {
        dst = data::_Decode(format, src, srcLength, dstLength);
    });
    ...
    if (ctype == data::CONTAINER_DATA)
    {
        ...
        if (dst != nullptr)
            Luax::CatchException(L, [&]() { data = instance()->NewByteData(dst, dstLength, true); });
        else
...
int Wrap_DataModule::Encode(lua_State * L)
{
    ...
    Luax::CatchException(L, [&]() {
        dst = data::_Encode(format, src, srcLength, dstLength, lineLength);
    });
...
int Wrap_DataModule::Pack(lua_State * L)
{
    ...
    if (ctype == data::CONTAINER_DATA)
    {
        ...
        Luax::CatchException(L, [&]() {
            data = instance()->NewByteData(buffer.nelems);
        });
