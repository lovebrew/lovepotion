...
namespace love
{
    ...
    struct StripSuffixIo : public PhysfsIo<StripSuffixIo>
    {
        ...
        static StripSuffixIo* create(const std::string& f)
        {
            return new StripSuffixIo(f);
        }
        ...
        StripSuffixIo(const StripSuffixIo& other) : StripSuffixIo(other.filename)
        {}
      ...
      private:
        StripSuffixIo(const std::string& f) : filename(f), file(nullptr)
        {
            try
            {
                file = new NativeFile(f, FileBase::MODE_READ);
            }
