...
namespace love
{
    ...
    {
        public:
            char * Compress(Compressor::Format format, const char * data, size_t size, int level, size_t & compressedSize) override
            {
                ...
                int status = this->zlibCompress(format, (Bytef *)compressedBytes, &destinationLength, (const Bytef *)data, (uLong)size, level);
            ...
            char * Decompress(Compressor::Format format, const char * data, size_t size, size_t & decompressedSize) override
            {
                if (!this->IsSupported(format))
                    throw love::Exception("Invalid format (expected zlib or gzip).");
                ...
                while (true)
                {
                    ...
                    int status = this->zlibDecompress(format, (Bytef *)rawBytes, &destinationLength, (const Bytef *)data, (uLong)size);
            ...
            {
                return format == Compressor::FORMAT_ZLIB || format == Compressor::FORMAT_GZIP || format == Compressor::FORMAT_DEFLATE;
            }

        private:
            uLong zlibCompressBound(Compressor::Format format, uLong sourceLength)
            {
                uLong size = sourceLength + (sourceLength >> 12) + (sourceLength >> 14) + (sourceLength >> 25) + 13;
            ...
            int zlibCompress(Compressor::Format format, Bytef * destination, uLongf * destinationLength, const Bytef * source, uLong sourceLength, int level)
            {
                z_stream stream = {};
            ...
            int zlibDecompress(Compressor::Format format, Bytef * destination, uLongf * destinationLength, const Bytef * source, uLong sourceLength)
            {
                z_stream stream = {};
