        ...
        public:
            char * Compress(Compressor::Format format, const char * data, size_t size, int level, size_t & compressedSize) override
            {
                ...
                *(uint32_t *)compressedBytes = (uint32_t)size;
                ...
                if (level > 8)
                    compressionSize = LZ4_compress_HC(data, compressedBytes + headerSize, (int)size, maxDestinationSize, LZ4HC_CLEVEL_DEFAULT);
                else
                    compressionSize = LZ4_compress_default(data, compressedBytes + headerSize, (int)size, maxDestinationSize);
            ...
            char * Decompress(Compressor::Format format, const char * data, size_t size, size_t & decompressedSize) override
            {
                ...
                */
                ...
                else
                {
                    ...
                    int result = LZ4_decompress_safe(data + headerSize, rawBytes, (int)(size - headerSize), rawSize);
